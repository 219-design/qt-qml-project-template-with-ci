...
pushd $DL_FOLDER >& /dev/null
  ...
  set +o pipefail
  yes | tools/bin/sdkmanager --sdk_root=$PWD "platforms;android-36" \
    || test $? -eq 141 # ignoring the pipefail caused by 'yes' https://stackoverflow.com/a/33026977/10278

  yes | tools/bin/sdkmanager --sdk_root=$PWD "build-tools;36.0.0" \
    || test $? -eq 141 # ignoring the pipefail caused by 'yes' https://stackoverflow.com/a/33026977/10278
  ...
  yes | tools/bin/sdkmanager --sdk_root=$PWD "ndk;27.2.12479018" \
    || test $? -eq 141 # ignoring the pipefail caused by 'yes' https://stackoverflow.com/a/33026977/10278
