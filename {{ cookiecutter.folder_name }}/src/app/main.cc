...
{
    {{ cookiecutter.cpp_namespace | replace('.', '::') }}::SetAppVersionStringForLogging( {{ cookiecutter.cpp_namespace | replace('.', '::') }}::GIT_HASH_WHEN_BUILT );
    {{ cookiecutter.cpp_namespace | replace('.', '::') }}::WhenOnIos_RerouteStdoutStderrToDeviceFilesystem();
    ...
    qSetMessagePattern( QString( "%{time yyyy-MM-dd hh:mm:ss.zzz} [QT-%{type}][v-" ) + {{ cookiecutter.cpp_namespace | replace('.', '::') }}::GIT_HASH_WHEN_BUILT
                        + "][thr:%{threadid}]%{if-category}%{category}: %{endif}%{file}(%{line}): %{message}" );

    if( !{{ cookiecutter.cpp_namespace | replace('.', '::') }}::OkToEnableAssertions() )
    {
    ...
    {{ cookiecutter.cpp_namespace | replace('.', '::') }}::ViewModelCollection vms( app, false /*cliArgsOnlyParseThenSkipErrorHandling*/ );
