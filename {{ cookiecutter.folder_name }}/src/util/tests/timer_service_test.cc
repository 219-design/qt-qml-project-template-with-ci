...
using {{ cookiecutter.cpp_namespace | replace('.', '::') }}::Timer;
using {{ cookiecutter.cpp_namespace | replace('.', '::') }}::TimerService;
...
TEST_F( TimerServiceTest, Interleaved )
{
...

    {
        ...
        {{ cookiecutter.cpp_namespace | replace('.', '::') }}::TimerService::AdvanceCurrentTime( 4000 );
    }
}
